...
Google's replacement, **Antigravity CLI** (`agy`), does not expose ACP on the CLI itself — there is no `agy acp` subcommand. Agent Client connects through Google's **ACP bridge** (`agy_acp_server.par`) as a built-in preset. See [Antigravity Setup](/agent-setup/antigravity).

The community [`agy-acp`](https://github.com/openabdev/openab/tree/main/agy-acp) npm adapter remains available as a [custom agent](/agent-setup/custom-agents) if you prefer that bridge instead of the registry `.par` binary.
