...
2. Export `GEMINI_API_KEY` in your environment (Obsidian inherits shell env on macOS/Linux; on Windows set user env vars and restart Obsidian), or put `GEMINI_API_KEY=…` in **Environment variables**. The health check accepts the key even if `settings.json` is missing.
