...
::: tip OpenCode, Kiro, and Antigravity
OpenCode, Kiro, and Antigravity are built-in presets — see [OpenCode Setup](./opencode), [Kiro Setup](./kiro), and [Antigravity Setup](./antigravity). If you previously configured them as custom agents (ids `opencode` / `kiro-cli` / `antigravity`), your settings migrate to the presets automatically where noted on each page.
:::
