...
3. **Exercise the UI** — Prefer the **floating chat** entry (`floatingChatEntry` / ribbon) when the feature is view-agnostic. For **floating chat**, **multi-session**, **tabs**, session switching, or tab-bar chrome: turn on Settings → Floating chat → **Enable floating chat tabs** (`enableFloatingChatTabs`) and test **multiple sessions as tabs inside one floating window**—not sidebar-only unless the change is sidebar-specific. Keep the **entire** chat window in frame (tab strip, composer, chip strip, send/queue icon). Move a corner floating window to the center if needed; start/stop the clip around the interaction (not minutes of idle). **Pause ≥2 seconds** on each distinct behavior state (tab switch, send, error, permission banner, etc.) before the next step so the recording showcases the change. Unique chip text and X must be readable. Use **`computerUse`** for GUI steps; use **RecordScreen** (or the walkthrough-artifacts skill) for MP4/WebP under `/opt/cursor/artifacts/`.
